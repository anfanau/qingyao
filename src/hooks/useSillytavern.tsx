...
function useSillytavernInternal(): UseSillytavernReturn {
  // --------------------- State --------------------------------
...
// ============================================================
// Context — ensures all components share ONE state instance
// ============================================================

const SillyTavernContext = createContext<UseSillytavernReturn | null>(null);

export function SillyTavernProvider({ children }: { children: ReactNode }) {
  const value = useSillytavernInternal();
  return (
 ...
 * MUST be used inside a <SillyTavernProvider>.
 */
export function useSillytavern(): UseSillytavernReturn {
  const ctx = useContext(SillyTavernContext);
  if (!ctx) {
    throw new Error(
      'useSillytavern() must be used inside <SillyTavernProvider>. ' +
      'Wrap your app root with <SillyTavernProvider> from the same module.'
    );
  }
  return ctx;
}
